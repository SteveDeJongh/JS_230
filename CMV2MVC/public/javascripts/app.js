...
class Model {
  ...
  // Set up, fetch contacts
  async fetchContactsData() {
    const response = await fetch(HOST + '/api/contacts');
    const data =  await response.json();
    this.contacts = this.tagsStringToArray(data);
  ...
  populateTags() {
    let allTags = this.contacts.flatMap(contact => contact.tags ? contact.tags : []);
    let tags = allTags.filter((tag, idx) => allTags.indexOf(tag) === idx);
    this.tags = tags.length > 0 ? tags : DEFAULT_TAGS; // default tag options present if no contacts with tags exist.
  }
